local function load_old_config()
    ...
    for _, opt in ipairs(opts) do
        local value = vim.g["chandrian_" .. opt]
        if value ~= nil then
            cfg[opt] = value
            using_old_config = true
            messages = messages .. "chandrian.nvim: option 'chandrian_" .. opt .. "' has been deprecated. See how to use the new configuration in README.md\n"
        end
    end

    if using_old_config then
        vim.schedule(function() vim.notify(messages, vim.log.levels.WARN, { title = "chandrian.nvim" }) end)
